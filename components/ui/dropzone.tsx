    ...
    <div className={cn('flex flex-col', className)}>
          ...
          >
            ...
            <div className="shrink grow flex flex-col items-start truncate">
              ...
              {file.errors.length > 0 ? (
                  ...
                  {file.errors
                    .map((e) =>
                      e.message.startsWith('File is larger than')
                        ? `Fichier plus grand que ${formatBytes(maxFileSize, 2)} (Taille: ${formatBytes(file.size, 2)})`
                        : e.message
                    )
                    .join(', ')}
                </p>
                ...
                <p className="text-xs text-destructive">Échec de l'upload: {fileError.message}</p>
              ) : isSuccessfullyUploaded ? (
                <p className="text-xs text-primary">Fichier uploadé avec succès</p>
              ) : (
      ...
      {exceedMaxFiles && (
        <p className="text-sm text-left mt-2 text-destructive">
          Vous ne pouvez uploader que {maxFiles} fichier{maxFiles > 1 ? 's' : ''} maximum, veuillez supprimer {files.length - maxFiles} fichier{files.length - maxFiles > 1 ? 's' : ''}.
        </p>
      )}
...
const DropzoneEmptyState = ({ className }: { className?: string }) => {
  ...
  return (
    <div className={cn('flex flex-col items-center gap-y-2', className)}>
      ...
      <p className="text-sm">
        Uploader{!!maxFiles && maxFiles > 1 ? ` ${maxFiles}` : ''} fichier{!maxFiles || maxFiles > 1 ? 's' : ''}
      </p>
